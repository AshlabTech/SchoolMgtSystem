				...
				if($num_rows1 > 0){
					...
					while($section_array1 = mysqli_fetch_array($query_section_payment)){
						$gender = $section_array1['sex'];
								 ...
								 $sql_section_payment2 = "select * from payment_details where school_section_id = '$school_section_id' and (sex='$gender') and status = '1'";
							
							$query_section_payment2 = mysqli_query($conn,$sql_section_payment2);
							$num_rows2= mysqli_num_rows($query_section_payment2);
								if($num_rows2 > 0){
									
											$sql_sum_amount = "select SUM(amount) from payment_details where school_section_id = '$school_section_id' and (sex='$gender') and status = '1'";
