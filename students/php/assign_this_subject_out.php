<?php 
	...
	$sub_section = "select * from subject where id = '$subject_id' and status ='1' LIMIT 1";
		...
		$sub_check = "select * from staff_subjects where subject_id = '$subject_id' and staff_info_id = '$staff_info_id'";
		...
		$num_rows_sub_check= mysqli_num_rows($sub_check_run);
			...
			if($num_rows_sub_check > 0){
				...
				$assign_query = mysqli_query($conn,"insert into staff_subjects (staff_info_id,subject_id,section_id) values ('$staff_info_id','$subject_id','$section_id')") or die(mysqli_error($conn));				
					if($assign_query){
						echo 'Assigned Successfully...';
					}else{
