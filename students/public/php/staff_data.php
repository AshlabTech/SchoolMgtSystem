<?php 
			include_once('connection.php');
				...
				//get the staff id from the staff_info table
				$sql_all_staff = "select * from staff_info s INNER JOIN  staff_login_info l ON s.staff_info_id = l.staff_info_id where s.staff_info_id = '$staff_info_id'";
				...
				$num_rows_all_staff = mysqli_num_rows($php_process_sql_all_staff);
					if($num_rows_all_staff > 0){
						...
						while($all_staff_array = mysqli_fetch_array($php_process_sql_all_staff)){
							...
							$level = isset($all_staff_array['level']) ? $all_staff_array['level'] : '' ;
					...
					$sql=mysqli_query($conn,"SELECT * FROM lga where local_id = '$lga_id'") ;
						$sql_get_state_row=mysqli_num_rows($sql);
							...
							//get social details
							$social = "select * from  staff_socials where staff_info_id = '$staff_info_id'";
							$social_run =  mysqli_query($conn,$social) or die(mysqli_error($conn));
							if(mysqli_num_rows($social_run) > 0){
									$social_arr = mysqli_fetch_array($social_run);
								$face_book_name = $social_arr['face_book_name'];
							}
