<?php 
			...
			// control page number
				if(isset($_POST["pn"])){
					...
					$countt = "select COUNT(student_class_id) from student_classes where class_id = '$class_id' and (status = '1' or status = '2')";
								...
								while($row=mysqli_fetch_assoc($sql)){
									$lga_title = $row['title'];
														...
														$delete_token = $student_info_id.','.$class_id.','.$pn;
															$tr .= '
																<tr>
																   <td width="1%"><a href="#" onclick="load_student_payment_details('.$student_info_id.')" type="button" class="btn btn-default"> <span class="glyphicon glyphicon-th-large" aria-hidden="true"></span> '.$sn++.'</a> </td>
