	...
	if(!isset($_POST['result_session'])){
		...
		//get class 
			...
			$class_section_num_rows =  mysqli_num_rows($class_section_run);
				...
				$sql_all_class = "select * from student_classes where class_id = '$result_class' and (status = '1' or status = '2') ORDER BY student_class_id ";
							...
							$student_info_id = $array_all_class['student_info_id'];
								...
								$num_rows_student_info = mysqli_num_rows($query_student_info);		
									...
									if($num_rows_student_info > 0){
										...
										$hash_id = md5($student_info_id);
										$token=$student_info_id.','.$result_session.','.$result_class.','.$result_term;
								
										$students_rows .='
												<div class="media" style="padding:0;margin:1px">
												 <div class="media-left" style=""> 
													<a href="#">
													  <img class="media-object img img-circle" src="../students_image_upload/'.$image_name.'" style="width:30px;height:30px" alt="$full_name">
													</a>
												  </div>
												 
												  <div class="media-body">
													<a  style="color:#1BBAE2;" href="view_student_result.php?token='.md5($student_info_id).'&cl='.md5($result_class).'&se='.$result_session.'&tr='.$result_term.'" target="_BLANK"><h6 class="media-heading text-left" style="margin-left:20px;margin-top:5px;"><strong>'.strtoupper($full_name).'</strong></h6></a>
														
												  </div>
												   <div class="media-right" style=""> 
												<a href="view_student_result.php?token='.md5($student_info_id).'&cl='.md5($result_class).'&se='.$result_session.'&tr='.$result_term.'" target="_BLANK"><button class="btn btn-info">View Result</button></a>
												  </div>
											</div>
