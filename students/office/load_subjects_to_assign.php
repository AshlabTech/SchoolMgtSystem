...
session_start();
		...
		$staffid = $_SESSION['staff_subject'];
					...
					$countt = "select COUNT(id) from  subject where school_section	= 3 OR school_section= 4 and status = '1'";
			...
			if($num_rows_all_subject > 0){
				$n=0;
					...
					$trr .= '<tr>
						...
						<td style="text-transform:uppercase;color:#06c;font-weight:bold;cursor:pointer" onclick="assign_this_subject_out('.$staffid.','.$subject_id.')">ADD</td>										
				</tr>';
				$sn++;
				$n++;
