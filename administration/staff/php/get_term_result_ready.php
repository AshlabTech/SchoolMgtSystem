<?php 
										...
										$class_section=  mysqli_query($conn,"select * from classes where class_id ='$class_id'");
										$cla = mysqli_fetch_array($class_section);
										$className =$cla['class_name'];
										$heads = "";
										if(strpos( $className, 'NUR')){
											$sql = "SELECT * FROM `school_head` as h inner join staff_info as s ON s.staff_info_id = h.staff_info_id WHERE h.id = 4";
											$run  = mysqli_query($conn, $sql);
											$clb = mysqli_fetch_array($run);
											$heads = $clb['title'].'. '.$clb['first_name'].' '.$clb['other_name'].' '.$clb['last_name'];
										}
										else if(strpos( $className, 'PRI')){
//												$ss = 'PRIMARY PUPILS';
											$sql = "SELECT * FROM `school_head` as h inner join staff_info as s ON s.staff_info_id = h.staff_info_id WHERE h.id = 6";
											$run  = mysqli_query($conn, $sql);
											$clb = mysqli_fetch_array($run);
											$heads = $clb['title'].'. '.$clb['first_name'].' '.$clb['other_name'].' '.$clb['last_name'];
										}
...
//											$ss = 'JUNIOR SECONDARY SCHOOL (J.S.S)';
											$sql = "SELECT * FROM `school_head` as h inner join staff_info as s ON s.staff_info_id = h.staff_info_id WHERE h.id = 5";
											$run  = mysqli_query($conn, $sql);
											$clb = mysqli_fetch_array($run);
											$heads = $clb['title'].'. '.$clb['first_name'].' '.$clb['other_name'].' '.$clb['last_name'];
										}
										...
										//Term result - calculate all the current score of the student for all the subject or term
											...
											$num_rows_check = mysqli_num_rows($check);
												if($num_rows_check > 0){
													...
													$insert_sql2 = "INSERT INTO term_result(student_info_id,session_id,class_id,term_id,total,headName)";
													$insert_sql2 .= "VALUES('$student_info_id','$session_id','$class_id','$term_id','$total_score_for_term', '$heads')";
												...
												if($term_id==3){
													...
													//if yes	
														...
														//if no
																$insert_yrly_score_sql = "INSERT INTO yearly_result(student_info_id,session_id,class_id,average_score,total_score, headName)";
																$insert_yrly_score_sql .= "VALUES('$student_info_id','$session_id','$class_id','$average_score','$total_yr_score','$heads')";
