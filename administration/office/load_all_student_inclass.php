    	...
    	$term_id = $tm['id'];
			...
			if(isset($_POST["token"])){
				...
				if(isset($_POST["pn"])){
					...
					$countt = "select COUNT(student_class_id) from student_classes where class_id = '$class_id' and (status = '1' or status = '2' ) and and session_id = '$session_id' ";
				...
				$sql_all_class = "select * from student_classes where class_id = '$class_id' and (status = '1' or status = '2' ) and session_id = '$session_id' ORDER BY student_class_id";
...
<div id="iframeCont" style="background: rgba(0,0,0,.3) !important;width: 100%; height: 100vh;">	
	<div id="iframeContInn">
		...
		<h4>
		<button class="btn btn-info" id="add_new_staff_btn" style="float:left:margin:5px;" onclick="load_all_student()" ><span class="glyphicon glyphicon-backward" ></span> Back</button>
			...
			<button  class="btn btn-primary " onclick="addM()" id='addM' data-toggle="modal" data-target="#addModal" style="float:right;"  ><span class="glyphicon glyphicon-download" ></span> Add members</button>
