				...
				$num_rows_all_staff = mysqli_num_rows($php_process_sql_all_staff);
					if($num_rows_all_staff > 0){
						...
						while($all_staff_array = mysqli_fetch_array($php_process_sql_all_staff)){
							$staff_title = $all_staff_array['title'];
							...
							$tr .= '<tr style="padding:0px;margin:0px">
										<td>'.$sn.'</td>
										 <td width="10%" class="text-center"><img class="img img-circle" onclick="load_change_staff_pics('.$staff_info_id.')" src="'.$user_pic.'" style="width:20px;height:20px"></td>
										<td>'.$staff_title.' '.$full_name.'</td>
							
										<td class="text-center">'.$staff_login_id.'</td>
										<td class="text-center" style="cursor:pointer">
											<span onclick="load_view_staff_details('.$staff_info_id.')" class="fa fa-eye"> view datails</span> 
...
?>															
												...
												<h4><i class="menu-icon fa fa-desktop"></i> All Staff Information</h4>
																		<div class="breadcrumb ace-save-state" id="breadcrumbs" style="margin:0">
																				<div  class="" id="" style="padding-bottom:">
