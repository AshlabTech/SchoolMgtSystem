...
<?php 
		...
		$sql_all_subject= "select * from subject where status ='1' ORDER BY school_section LIMIT $limit";
			...
			if($num_rows_all_subject > 0){
				$n=0;
					...
					$trr .= '<tr>
						...
						<td style="text-transform:uppercase"><button onclick="(function(){$.post(\'delete_subject.php\',{id:'.$subject_id.'},function(data){$(\'#loader\').hide(); if(data==200){system_setup();}else{console.log(data);} }); $(\'#loader\').show()})()" class="btn btn-danger">&times</button></td>										
				</tr>';
				$sn++;
				$n++;
...
<table class="table table-bordered table-responsive table-hover" style='margin:20px'>	
	<thead>
					<tr>
						...
						<td style="text-transform:uppercase">x</td>											
				</tr>
	</thead>
	<tbody>
		<?php echo $trr; ?>
