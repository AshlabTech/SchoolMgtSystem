				...
				$num_rows_all_class = mysqli_num_rows($query_all_class);
					if($num_rows_all_class > 0){
						...
						while($array_all_class = mysqli_fetch_array($query_all_class)){
							...
							{
								$sql_all_class2 = "select * from student_classes where class_id = '$class_id' and session_id = '$session_id' and  (status = '1' or status='2')";
							}else{
								$sql_all_class2 = "select * from student_classes where class_id = '$class_id' and session_id = '$session_id' and  (status != '0')";
							}
											...
											$no_that_pay_zero = $total_students_inclass - ($gno_that_paid_all + $half_paid);
												...
												$tr .='
													<tr>
														...
														<td class="text-center"><a href="#" onclick="return false" onmousedown="view_all_paid_inclass('.$class_id.','.$session_id.','.$term_id.')">View All</a></td>
													</tr>
												';
