...
if (isset($_POST['save'])) {
	$head_id = mysqli_real_escape_string($conn,$_POST['header']);
	$staff = mysqli_real_escape_string($conn,$_POST['staff']);
	$chk = $conn->query("SELECT * FROM `staff_info` WHERE category = '$head_id'")OR die(mysqli_error($conn));
	if ($chk->num_rows>0) {
...
//		echo $ch_id;
		$up = $conn->query("update staff_info set category='0' where staff_info_id ='$ch_id'")OR die(mysqli_error($conn));
	}

	$up = $conn->query("update staff_info set category='$head_id' where staff_info_id = '$staff'") OR die(mysqli_error($conn));
	if ($up) {
        ...
        <div class="form-group">
				...
				<div class="col-sm-5">
					<select name="staff" class="form-control" required>
						<option value=""></option>
						<?php
						$staffs = $conn->query('select * from staff_info where status = "1"');
							if ($staffs->num_rows >0) {	
								
								while ($row = $staffs->fetch_assoc()) {
									?>
									<option value="<?php echo $row['staff_info_id']; ?>"><?= $row['first_name'].' '. $row['other_name']. ' '. $row['last_name']?></option>
									<?php
								}
							}
						?>
