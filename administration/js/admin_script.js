...
function move_student_passport_to_folder(){
	...
	function load_promote_student_by_session_class(){
		...
		alert(class_id);
		getId('promotion_wrap').innerHTML = '<div class="text-center" style="height:300px;line-height:300px"><img src="../../images/ajax-loader.gif"></div>';
					
			$.post('../office/load_promote_student_by_session_class.php',{p_session:p_session,p_class:class_id},
					function(response,error){
...
function close_change_staff_pics(token){
	...
	function load_edit_student_form(token){
		...
		function display_classes_payment(){
			...
			getId("make_payement_student_search_wrap").innerHTML = "<h4 style='text-align:center'><img src='../../images/ajax-loader.gif'></h4>";
			$.post("display_classes_payment.php",{session_id:session_id,term_id:term_id},function(response){
				getId("make_payement_student_search_wrap").innerHTML = response;
			})    
		}
		...
		if(con){
			...
			getId("make_payement_student_search_wrap").innerHTML = "<h4 style='text-align:center'><img src='../../images/ajax-loader.gif'></h4>";
			$.post("display_deptors_list_by_classes.php",{session_id:session_id,term_id:term_id},function(response){
				getId("make_payement_student_search_wrap").innerHTML = response;
			})    
		}
		...
		{ 
		  ...
		  var docprint=window.open("","",disp_setting);  
			docprint.document.write('<html><head><title>.::ELMAASUM</title> <link rel="stylesheet" href="http://elmaasum.com/css/bootstrap.css">');
		...
		function display_term_payment_summary(){
			...
			getId("make_payement_student_search_wrap").innerHTML = "<h4 style='text-align:center'><img src='../../images/ajax-loader.gif'></h4>";
			$.post("display_term_payment_summary.php",{session_id:session_id,term_id:term_id},function(response){
				getId("make_payement_student_search_wrap").innerHTML = response;
			})    
		}
		...
		function display_session_payment_summary(){
			...
			getId("make_payement_student_search_wrap").innerHTML = "<h4 style='text-align:center'><img src='../../images/ajax-loader.gif'></h4>";
			$.post("display_session_payment_summary.php",{session_id:session_id,term_id:term_id},function(response){
				getId("make_payement_student_search_wrap").innerHTML = response;
			})    
		}
		function view_all_paid_inclass(token,session_id,term_id){
			...
			getId("make_payement_student_search_wrap").innerHTML = "<h4 style='text-align:center'><img src='../../images/ajax-loader.gif'></h4>";
			$.post("view_all_paid_inclass.php",{token:token,session_id:session_id,term_id:term_id},function(response){
				getId("make_payement_student_search_wrap").innerHTML = response;
			})    
		}
		...
		function score_entry_frame(){
		 document.getElementById("display_content").innerHTML = '<center><img src="../../images/ajax-loader.gif"><br> please, wait...</center>';
		   $.post("score_time_frame_container.php",{token:'token'},function(response,error){
				getId('display_content').innerHTML = response;
			});
}
		function laod_manage_form_teacher(){
		 document.getElementById("display_content").innerHTML = '<center><img src="../../images/ajax-loader.gif"><br> please, wait...</center>';
		   $.post("../staff/pages/laod_manage_form_teacher.php",{token:'token'},function(response,error){
				getId('display_content').innerHTML = response;
			});
	}
	...
	var cm1=0; selected1 = ''; selected='';
var cm=0; var selected =''; 
    ...
    var subSelected = [];
     ...
     function selchecked (id,idchk,cl, clchk){
                ...
                if ($(idchk).is(':checked')) {          
                	...
                	Selected = $('.'+cl).map((_,el) => el.value).get();   
...
//------------------Delete selected student from class
	$('#rmvM').click(function(){
		if(selected!=''){
			...
			Swal.fire({
				...
				$.get('inajx.php',{selected:selected,type:'remveM'}, function(data){
					$("#load1").hide();
				});
					$("#load1").show();
            	}
	...
	//$('#transM').click(function(){
		if(selected1!=''){
			$('#iframe').attr('src','ques/transferbox.php?branch='+class_sel+'&sel='+selected1+'&t='+termId+'&b='+classId+'&s='+sessionId);
	...
	function importD(sel,b,t,s){
		...
		Selected = [];
		if(sel != null){
			$.get('ques/imports.php',{type:'imports',sel:sel,b:b,t:t,s:s}, function(data){
			$("#load1").hide();	
			console.log(data);
